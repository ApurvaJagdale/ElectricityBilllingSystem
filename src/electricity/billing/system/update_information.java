...
public class update_information extends JFrame implements ActionListener {
	JLabel nametext;
	JTextField addressText,cityText,stateText,emailText,phoneText,meterText;
	String meter;
	...
	update_information(String meter){
		...
		nametext = new JLabel("");
		nametext.setBounds(150,90,200,20);
		add(nametext);
		
		
		JLabel meterNo = new JLabel("Meter Number");
		meterNo.setBounds(30,120,110,20);
		add(meterNo);
		...
		address.setBounds(30,150,110,20);
		add(address);
		...
		try {
			...
			System.out.println("Exception in update information: ");
			e.printStackTrace();
		}
	...
	public void actionPerformed(ActionEvent e) {
		if(e.getSource()==update) {
			...
			try {
				database c = new database();
				System.out.println("update new_customer set address ='"+saddress+"', city='"+scity+"',state  ='"+sstate+"',email = '"+semail+"',phone_no ='"+sphone+"' where meter_no = '"+meter+"'");
	...
	//Alter table new_customer alter column address TYPE varchar(100);
